...
const Group = () => {
    ...
    const getStandingsHeaders = (games: GameType[]) => {
        ...
        const nbRemainingGames =
            games[games.length - 1]?.id % NUMBER_GROUPED_GAMES
                ...
                <div>
                    ...
                    {group.games.map((game) => (
                        <Button
                            key={game.id}
                            onClick={() =>
                                navigateToGameResult({ gameId: game.id })
                            }
                        >
                            {t('Group.game', { id: game.id })}
                        </Button>
                            ...
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        defaultChecked
                                        onChange={(event) =>
                                            setGroupGames(event.target.checked)
                                        }
                                        ...
                                        >
                                            ...
                                            {groupedGames &&
                                                ...
                                                groupedGames.map((group) => {
                                                    return (
                                                        <TableCell
                                                            key={`score-game-player-${player.name}`}
                                                        >
                                                            {
                                                                group.find(
                                                                    (
