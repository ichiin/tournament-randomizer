...
const useGroupRandomizer = () => {
    ...
    const generateGroups = () => {
        const seededPlayers = [...playerList].filter(
            (player) => player.isSeeded,
        )
        ...
        if (groupSize) {
            ...
            while (i < shuffledPlayersList.length) {
                const nbSeeded =
                    seededGeneratedGroups[seededPlayersIndex].length
                ...
                const filledGroup = shuffledPlayersList.slice(
                    i,
                    i + nbPlayersToFill,
                )
    ...
    useEffect(() => {
        const fetchPlayers = async () => {
            console.log(
                'getting the list of players from the provided channel...',
            )
